...
def day_of_week(event_timestamp: pd.Series) -> pd.Series:
    ...
    return pd.Series(event_timestamp.dt.day_of_week, name="day_of_week", index=event_timestamp.index)
...
def trips_stats_3h(
    ...
    df = pd.concat([
        event_timestamp, driver_id, rider_id, trip_dist, day_of_week,
        is_weekend, month, avg_trip_dist_rolling_3h,
        max_trip_dist_rolling_3h, min_trip_dist_rolling_3h,
        percentile_dist_rolling_3h,
    ], axis=1)
    return df


def trips_stats_3h_materialize(
    trips_stats_3h: pd.DataFrame,
    trips_stats_3h_push: PushSource,
    feature_store: FeatureStore,
) -> None:
    feature_store.push(
        trips_stats_3h_push.name,
        trips_stats_3h,
        to=PushMode.ONLINE
    )
